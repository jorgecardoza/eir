...
public class RemoveImeiListLoad implements ServiceLogic {
	...
	@Override
	public void setMap(GenericDto removeImeiListMap) {
		this.eirUtil = (EirUtil) removeImeiListMap.get("util");
		this.env = (Environment) removeImeiListMap.get("env");
		this.log = (Loggers) removeImeiListMap.get("log");
		this.orchestratorMethod = (OrchestratorMethod) removeImeiListMap.get("orchestratorMethod");
		this.requestData = (GenericDto) removeImeiListMap.get("requestData");
	...
	public GenericDto execute() {
		...
		try {
			...
			if (eirUtil.stringValue(requestData.get("list")).equals(""))//antes tenia imsi
				throw new ServiceException(3, "Parmetro de entrada requerido imsi no esta presente");

			String list = String.valueOf(requestData.get("list"));
			if (isNullOrEmpty(list))
				throw new ServiceException(1, "Campo list no puede ser vacio");

			List<String> validList = Arrays.asList("black", "white", "grey");
			if (!validList.contains(list))
				throw new ServiceException(1, String.format("Error campo list solo puede ser $s", validList));
...
//			headers.add(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE);
			...
			StringBuilder builder = new StringBuilder(env.getProperty("eir.url.remove"));
...
//			responseData.setProperty("message", statusCode.getReasonPhrase());
			...
			if (HttpStatus.OK.equals(statusCode)) {
				if (!responseData.containsKey("result"))
					responseData.setProperty("result", "deleted");
			} else {
				if (!responseData.containsKey("result"))
					responseData.setProperty("result", "not deleted");
			}
	...
	private boolean isNullOrEmpty(Object object) {
		...
		return "null".equalsIgnoreCase(value) || "".equals(value);
	}
