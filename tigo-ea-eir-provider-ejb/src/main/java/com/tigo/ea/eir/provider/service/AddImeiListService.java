...
@Service
public class AddImeiListService extends AbstractConvertibleService<GenericDto, GenericDto> {
	@Autowired
	...
	protected GenericDto execute(ProviderRequest arg0, ProviderResponse response, GenericDto requestData) {
		log.setMethodName("AddImeiListService");

		ServiceLogic serviceLogic = (ServiceLogic) classFactory.loadClass("AddImeiListLoad");
		GenericDto addImeiListMap = new GenericDto();
		addImeiListMap.put("util", util);
		addImeiListMap.put("env", env);
		addImeiListMap.put("log", log);
		addImeiListMap.put("strainerTsb", strainerTsb);
		addImeiListMap.put("orchestratorMethod", orchestratorMethod);
		addImeiListMap.put("requestData", requestData);
		serviceLogic.setMap(addImeiListMap);
